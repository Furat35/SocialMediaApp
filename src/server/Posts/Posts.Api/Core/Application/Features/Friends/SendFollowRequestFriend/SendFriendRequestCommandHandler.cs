...
{
    public class SendFollowRequestCommandHandler(IFriendRepository followRepository, IHttpContextAccessor httpContextAccessor)
        : IRequestHandler<SendFollowRequestCommand, ResponseDto<bool>>
    {
        public async Task<ResponseDto<bool>> Handle(SendFollowRequestCommand request, CancellationToken cancellationToken)
        {
            ...
            var followExists = await followRepository
            ...
            await followRepository.AddAsync(new Domain.Entities.Friend
            {
